import {validateHistory} from './qc.mjs';
...
export async function openCaseFile(text, nested=false){
 ...
 if(nested&&n.qc!==undefined)throw Error('Nested QC histories are not supported.');
 ...
 const reviewScope=c.review_scope||{coverage:c.coverage||'',queried_addresses:c.queried_addresses||[]};
 if(typeof reviewScope.coverage!=='string'||reviewScope.coverage.length>5000||!Array.isArray(reviewScope.queried_addresses)||reviewScope.queried_addresses.length>5||reviewScope.queried_addresses.some(a=>!addr(a)))throw Error('Invalid original review scope.');
 loaded.review_scope={coverage:reviewScope.coverage,queried_addresses:reviewScope.queried_addresses};
 if(!nested){notes.qc=validateHistory(n.qc);for(const r of notes.qc){const scope=r.snapshot.scope;if(scope.seed!==seed)throw Error('QC submission belongs to another subject.');await openCaseFile(JSON.stringify({case:{manifest:{...m,...scope},events:r.snapshot.events},analyst_notes:r.snapshot.notes}),true);}}
